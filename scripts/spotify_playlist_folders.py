...
# -*- coding: utf-8 -*-
"""
Read Spotify playlist folder hierarchy from the desktop app's local cache.

Vendored from https://github.com/mikez/spotify-folders (MIT).
"""
...
def find_folder_by_name(node: dict, name: str) -> dict | None:
    """Return the first folder node whose name matches (case-insensitive)."""
    if node.get("type") == "folder" and str(node.get("name", "")).lower() == name.lower():
        return node
    for child in node.get("children", []):
        found = find_folder_by_name(child, name)
        if found:
            return found
    return None


def playlist_ids_in_folder(
    folder_name: str,
    account: str | None = None,
    cachedir: str = PERSISTENT_CACHE_PATH,
) -> list[str]:
    """
    Playlist Spotify IDs inside a named folder in the user's library.

    The Web API does not expose folders; this reads the desktop client's cache.
    """
    user_id, raw_rootlist = get_leveldb_rootlist(account, cachedir)
    if not raw_rootlist:
        raise RuntimeError(
            "No playlist data in the Spotify cache. Open the Spotify app, "
            "confirm your library synced, then try again."
        )
    tree = parse(raw_rootlist, user_id or "unknown")
    folder = find_folder_by_name(tree, folder_name)
    if not folder:
        raise RuntimeError(
            f'Playlist folder "{folder_name}" not found in the Spotify cache. '
            "Check the folder name in Spotify (case-insensitive) and try again."
        )
    ids: list[str] = []
    for child in folder.get("children", []):
        if child.get("type") != "playlist":
            continue
        uri = str(child.get("uri", ""))
